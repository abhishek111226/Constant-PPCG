...
void isl_ast_expr_modify_access_to_const_call(__isl_keep isl_ast_expr * ast_expr)
{
	isl_ctx * ctx = isl_ast_expr_get_ctx(ast_expr);
	const char  * tmp =isl_id_get_name(ast_expr->u.op.args[0]->u.id);	
	char * prefix="const_";
	...
	strcat(arg2_char,tmp);
	isl_id * arg2_id = isl_id_alloc(ctx,arg2_char,NULL);	
	isl_ast_expr_free(ast_expr->u.op.args[0]);
	ast_expr->u.op.args[0] =isl_ast_expr_from_id(arg2_id);
	free(arg2_char);
}
	...
	isl_bool (*fn)(__isl_keep isl_ast_node *node, void *user), void *user)
{
	...
	case isl_ast_node_for:
		...
		node = node->u.f.body;
		isl_stat rtn= isl_ast_node_foreach_descendant_top_down(node, fn, user);
		if(!arg->is_annotating)		
			arg->kernel_loop_depth_count--;
