...
__isl_give isl_printer * copy_data_from_device_to_device(__isl_take isl_printer * p,struct ppcg_kernel *kernel)
{
	...
	{
		if(kernel->array[i].is_written && (kernel->array[i].array->is_in_texture || kernel->array[i].array->is_in_surface))
		{
...
__isl_give isl_printer * copy_array_from_device_constant_memory(__isl_take isl_printer *p, struct gpu_array_info *array)
{
		printf("codegen for const mem");
		p = isl_printer_start_line(p);
		p = isl_printer_print_str(p, "cudaCheckReturn(cudaMemcpyToSymbol(const_");
		p = isl_printer_print_str(p, array->name);
		p = isl_printer_print_str(p, ", ");
		p = isl_printer_print_str(p, array->name);
		p = isl_printer_print_str(p, ", ");
...
__isl_give isl_printer * print_non_linearized_declaration_argument_const(
	__isl_take isl_printer *p, struct gpu_array_info *array)
{
	p = isl_printer_print_str(p, array->type);
	p = isl_printer_print_str(p, " ");
	p = isl_printer_print_str(p, "const_");
	p = isl_printer_print_ast_expr(p, array->bound_expr);

	return p;
}

__isl_give isl_printer *  print_constant_declaration(__isl_take isl_printer *p, struct gpu_array_info * array)
{
	// __constant__ float cangle[360];
	p = isl_printer_start_line(p);
	p = isl_printer_print_str(p,"__constant__ ");
	print_non_linearized_declaration_argument_const(p, array);
	p = isl_printer_print_str(p,";");
	p = isl_printer_end_line(p);	
	return p;	
	//p = isl_printer_print_str(p, array->type);
...
isl_printer * print_texture_or_surface_decl(isl_printer *p, struct gpu_prog * prog)
{
	...
	for(i=0;i<prog->n_array;i++)
	{
		...
		else if(prog->array[i].is_in_constant)
			p = print_constant_declaration(p,&prog->array[i]);
	} 
	p=isl_printer_print_str(p,"\n #endif \n");
...
__isl_give isl_printer * setup_constant_memory(__isl_take isl_printer *p, struct gpu_prog *prog)
{
	int i;
	for (i = 0; i < prog->n_array; ++i) 
	{
		if(prog->array[i].is_in_constant)
		{
			p = copy_array_from_device_constant_memory(p,&prog->array[i]);
		}	
	}
	p = isl_printer_start_line(p);
	p = isl_printer_end_line(p);
...
__isl_give isl_printer *gpu_array_info_print_call_argument_nodev(
	__isl_take isl_printer *p, struct gpu_array_info *array)
{
...
__isl_give isl_printer * print_call_constant_setup(__isl_take isl_printer *p, struct gpu_prog *prog)
{
	int i;
	p = isl_printer_start_line(p);
	p = isl_printer_print_str(p, "init_const_scop_");
	p = isl_printer_print_int(p, const_scop_counter);
	p = isl_printer_print_str(p, "(");
	...
	for (i = 0; i < prog->n_array; ++i) 
	{
		if(prog->array[i].is_in_constant)
		{
			...
			p = gpu_array_info_print_call_argument_nodev(p,&prog->array[i]);
		}	
	}
	p = isl_printer_print_str(p,");");
	p = isl_printer_end_line(p);
...
/* is_decl: Declaration to print? True call to print : False */
__isl_give isl_printer * print_defn_constant_setup(__isl_take isl_printer *p, struct gpu_prog *prog, bool is_only_decl)
{
	int i;
	p = isl_printer_start_line(p);
	p = isl_printer_print_str(p, "void init_const_scop_");
	p = isl_printer_print_int(p, const_scop_counter);
	p = isl_printer_print_str(p, "(");
	bool not_first = false;
	for (i = 0; i < prog->n_array; ++i) 
	...
	p = isl_printer_end_line(p);
	p = isl_printer_start_line(p);
	p = setup_constant_memory(p, prog);
	p = isl_printer_end_line(p);
	p = isl_printer_start_line(p);
	p = isl_printer_print_str(p, "}");
	p = isl_printer_end_line(p);
...
void clear_texture_flags(struct gpu_prog * prog)
{
	...
	printf("clear texture flags done !");
	fflush(stdout);
} 
...
{
	if(candidate->array_info->is_in_constant)
	{
		if(!is_access_write)
		{
			isl_ast_expr *ast_expr;
			ast_expr = isl_id_to_ast_expr_get(ref2expr,pet_expr_access_get_ref_id(expr));
			isl_ast_expr_modify_access_to_const_call(ast_expr);
			isl_ast_expr_free(ast_expr);
			return -1;			
		}
		else
		{
			printf("\n [ERR] Array in constant memory but written !");
			return -1;
		}
	}	
...
void decide_surface_or_texture_or_constant(struct kernel_analysis * kernel_analysis)
{
	int i,j;
	struct candidate_info * analysis_tableu = kernel_analysis->analysis_tableu;
	bool is_read_only;
	...
	for(i=0;i<kernel_analysis->candidates_cnt;i++)
	{
		...
		for(j=0;j<kernel_analysis->kernel_cnt;j++)
		{
			if(analysis_tableu[i].latest_access[j]==invalid)
			{
				...
				analysis_tableu[i].array_info->is_in_constant=false;
		...
		if(is_marked_invalid)
			continue;
		// FIXME: These are just placeholders. We are focusing on const memory optimizations.		
		analysis_tableu[i].array_info->is_in_texture=false;
		analysis_tableu[i].array_info->is_in_surface=false;
		if(!is_read_only)
		{
		...
		{
			analysis_tableu[i].array_info->is_in_constant=true;
		}
		/*else
		{
	...
	printf("\n\t\t ------ Decision: texture or surface or constant -------------");
	for(i=0;i<kernel_analysis->candidates_cnt;i++)
	{		
		printf("\n\t %s \t %d \t %d \t %d \t %d ",analysis_tableu[i].array_info->name,analysis_tableu[i].array_info->is_in_texture ,analysis_tableu[i].array_info->is_in_surface, analysis_tableu[i].array_info->is_in_constant, analysis_tableu[i].array_info->size);
	}
	printf("\n\n");
}
...
{
	int i;
	printf("\n candidates: %s",array_name);
	for(i=0;i<candidates_cnt;i++)
	{
		printf("\n checking with %s",analysis_tableu[i].array_info->name);
...
bool constant_profitability(isl_ast_expr *ast_expr, struct kernel_analysis * kernel_analysis)
{
	//int b_n = isl_id_list_n_id(kernel_analysis->block_ids);
	int t_n = isl_id_list_n_id(kernel_analysis->thread_ids);
	int i;
	isl_ctx * ctx= isl_ast_expr_get_ctx(ast_expr);
	isl_printer * p = isl_printer_to_str(ctx);
	char * access = isl_printer_get_str(isl_printer_print_ast_expr(p,ast_expr));
	for(i=0;i< t_n; i++)
	{
		isl_id *id;
		id = isl_id_list_get_id(kernel_analysis->thread_ids, i);
		const char * name = isl_id_get_name(id);
		if(strstr(access, name) != NULL) 
		{
			printf(" \n %s within %s ", name, access);
			isl_id_free(id);
			isl_printer_free(p);
			return false;		
		}
	...
	{
		isl_id *id;
		id = isl_id_list_get_id(kernel_analysis->block_ids, i);
		const char * name = isl_id_get_name(id);
		if(strstr(access, name) != NULL) 
		{
			printf(" \n %s within %s ", name, access);
...
int for_each_array_access(__isl_keep pet_expr *expr, void *user)
{
	...
	if(array_name!=NULL)
	{
		...
		if(array_index>=0)
		{
			...
			else if(kernel_analysis->is_costing)
			{
				if(kernel_analysis->analysis_tableu[array_index].array_info->is_in_constant)
				{
					printf("--- Constant Cost model ------");
					if(!constant_profitability(ast_expr, kernel_analysis))
					{
					kernel_analysis->analysis_tableu[array_index].array_info->is_in_constant = false;
					}
				}
			}
			else
		...
		{
			printf("\n Nothing to annotate for array %s ", array_name);
		}
	}
	isl_ast_expr_free(ast_expr);
	return 0;
...
void traverse_kernel(struct ppcg_kernel *kernel, struct kernel_analysis * kernel_analysis)
{
	...
	if(kernel_analysis->is_costing)
	{
		kernel_analysis->block_ids = kernel->block_ids;
		kernel_analysis->thread_ids = kernel->thread_ids;
...
void cost_model(isl_ast_node *tree, struct kernel_analysis * kernel_analysis_arg)
{
	//isl_id_list * block_ids  = kernel->block_ids;
	//isl_id_list * thread_ids  = kernel->thread_ids;
	kernel_analysis_arg->is_costing = true;
	kernel_analysis_arg->is_annotating = false;
	traverse_gpu_code(tree,kernel_analysis_arg);

	/*int n_array = kernel->n_array;
	struct gpu_local_array_info *array = kernel->array;
	int i;
	for(i=0;i< n_array; i++)
	{
		struct gpu_local_array_info temp = array[i];
		printf(" \n Array : %s \n",temp.array->name);
		isl_printer * p = isl_printer_to_str(kernel->ctx);
		if(temp.n_index)	
		{	
			int n_group = temp.n_group;
			struct gpu_array_ref_group **groups = temp.groups;
			int j;			
			for(j=0;j< n_group; j++)
			{
			
				int n_ref = (*groups)[j].n_ref;
				struct gpu_stmt_access **refs = (*groups)[j].refs;	
				int k;
				for(k=0;k<n_ref; k++)
				{
					printf("\n\n here");	
					fflush(stdout);
					struct gpu_stmt_access temp = (*refs)[k];
					isl_map *access = temp.access;
					isl_map *tagged_access = temp.tagged_access;
					char * new_var =  isl_printer_get_str(isl_printer_print_map (p,access));
				char * new_var_reln =  isl_printer_get_str(isl_printer_print_map (p,tagged_access));	
					printf("\n\n %s", new_var);
					fflush(stdout);				
				}	
			}
			//char * new_var =  isl_printer_get_str(isl_printer_print_ast_expr(p, temp.bound_expr));
		
		//char * new_var_reln=  isl_printer_get_str(isl_printer_print_map (p,access_reln));
			//printf("1st one %d %s \n\n", temp.n_index ,new_var);
		//printf("2nd one \n\n %s \n\n", new_var_reln);
		}	
		isl_printer_free(p);
	}*/				
...
void decide_profitability(isl_ast_node *tree, struct kernel_analysis * kernel_analysis)
{
	int i,j;
	struct candidate_info * analysis_tableu = kernel_analysis->analysis_tableu;
	/*for(i=0;i<kernel_analysis->candidates_cnt;i++)
	{
		...
		{
			//texture_cost_model(analysis_tableu[i].array_info, analysis_tableu[i].array_info->is_in_texture)
		}
		else if (analysis_tableu[i].array_info->is_in_constant)
		{
			constant_cost_model(analysis_tableu[i].array_info);
			//if(!constant_cost_model(analysis_tableu[i].array_info))
			//{
			//	continue;						
			//}	
		}
	...
	cost_model(tree, kernel_analysis);
	printf("\n\t\t ------ constant memory cost model -------------");
	for(i=0;i<kernel_analysis->candidates_cnt;i++)
	{		
		printf("\n\t %s \t %d \t %d \t %d ",analysis_tableu[i].array_info->name,analysis_tableu[i].array_info->is_in_texture ,analysis_tableu[i].array_info->is_in_surface, analysis_tableu[i].array_info->is_in_constant);
	}
	printf("\n\n");
}
...
struct kernel_analysis * setup_candidate_arrays(struct gpu_prog *prog)
{
	...
	if(prog->scop!=NULL)
	{
		// FIXME: Unused variable candidate_list needs to change pet code to remove initialization etc.
		//struct texture_candidate **candidate_list = prog->scop->pet->candidates;
		...
		printf(" \n Candidates const mem are : %d",no_of_candidates);
		...
		{	
			if(prog->array[i].n_index)
			{
				printf("\n\t\t %s %d ",prog->array[i].name,prog->array[i].n_index);
				setup_candidate_info(analysis_table,count++,&prog->array[i],no_of_kernels);
			}		
		}	
...
void constant_optimization(struct gpu_prog *prog,isl_ast_node *tree)
{
	//is_ccp_greater_than_two = (prog->scop->options->use_surface_memory)? true : false;
	//is_ccp_greater_than_two? printf("\n Using surfaces \n") : printf("\n Not Using surfaces \n");

	const_mem_opt = prog->scop->options->use_constant_memory;
	if(!const_mem_opt) {
		return ;
	}
	printf("Here");
	...
	decide_surface_or_texture_or_constant(kernel_analysis_arg);

	decide_profitability(tree,kernel_analysis_arg);	
