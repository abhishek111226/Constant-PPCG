...
#define cudaCheckKernel() \
  do { \
   ...
   cudaCheckReturn(cudaMalloc((void **) &dev_alpha, sizeof(double)));
   cudaCheckReturn(cudaMalloc((void **) &dev_beta, sizeof(double)));
   cudaCheckReturn(cudaMalloc((void **) &dev_r, (120) * sizeof(double)));
   cudaCheckReturn(cudaMalloc((void **) &dev_sum, sizeof(double)));
   cudaCheckReturn(cudaMalloc((void **) &dev_y, (120) * sizeof(double)));
   cudaCheckReturn(cudaMalloc((void **) &dev_z, (119) * sizeof(double)));
   
   
   beta = 1.;
   alpha = (-r[0]);
   y[0] = (-r[0]);
   cudaCheckReturn(cudaMemcpy(dev_alpha, &alpha, sizeof(double), cudaMemcpyHostToDevice));
   cudaCheckReturn(cudaMemcpy(dev_beta, &beta, sizeof(double), cudaMemcpyHostToDevice));
   cudaCheckReturn(cudaMemcpy(dev_r, r, (120) * sizeof(double), cudaMemcpyHostToDevice));
