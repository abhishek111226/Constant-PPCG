...
//	p = setup_constant_memory(p, prog);
	if(const_mem_opt) {
		p = print_call_constant_setup(p, prog);	
	}
	return p;
}
	...
	struct gpu_types *types, void *user)
{
	...
	constant_optimization(prog,tree);
	...
	if(const_mem_opt) {
		tp = print_defn_constant_setup(tp, prog, true);
	}
	isl_printer_free(tp);

	if(const_mem_opt) {
		isl_printer *const_setup_call;
		const_setup_call = isl_printer_to_file(isl_printer_get_ctx(p), cuda->kernel_c);
		const_setup_call = isl_printer_set_output_format(const_setup_call, ISL_FORMAT_C);
		const_setup_call = print_defn_constant_setup(const_setup_call, prog, false);
		isl_printer_free(const_setup_call);
		const_scop_counter++;
	}
